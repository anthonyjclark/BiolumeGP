...
public class Biolume {
	...
	public static final int GENOME_SIZE_MAX = (CC.EXE_SIZE_MAX + Variable.NUM_VARS);
	...
	public void step() {
		...
		this.add_energy(-1.0 * CC.ENERGY_DECAY); 
	...
	public void display() {
		...
		this.sim.fill(0,0,0);
		this.sim.rect(this.address.x * Simulator.X_DISP, this.address.y * Simulator.Y_DISP, CC.B_WIDTH, CC.B_HEIGHT);
		
		// draw the Biolume
		if (this.is_reproducing()) {
			
			this.sim.image(Simulator.repro, this.address.x  * Simulator.X_DISP, this.address.y * Simulator.Y_DISP, CC.B_WIDTH, CC.B_HEIGHT);
			...
			this.sim.rect(this.address.x  * Simulator.X_DISP, this.address.y * Simulator.Y_DISP, CC.B_WIDTH/2, CC.B_HEIGHT);
			// set LED1 to specific color
			this.sim.fill(this.display_current[3], this.display_current[4], this.display_current[5]);
			this.sim.rect(this.address.x  * Simulator.X_DISP+CC.B_WIDTH/2, this.address.y * Simulator.Y_DISP, CC.B_WIDTH/2, CC.B_HEIGHT);
			// set speaker status
			if (speaker_on) this.sim.image(Simulator.sound, this.address.x * Simulator.X_DISP + CC.B_WIDTH/4, this.address.y * Simulator.Y_DISP + CC.B_HEIGHT/8, CC.B_WIDTH/2, CC.B_HEIGHT/2);
			// energy display
			if (this.get_energy() <= 0)
				this.sim.image(Simulator.battery0, this.address.x * Simulator.X_DISP, this.address.y * Simulator.Y_DISP + CC.B_HEIGHT *9/16, CC.B_WIDTH, CC.B_HEIGHT *9/16);
			else if (this.get_energy() <= 0.25 * CC.ENERGY_MAX)
				this.sim.image(Simulator.battery1, this.address.x * Simulator.X_DISP, this.address.y * Simulator.Y_DISP + CC.B_HEIGHT *9/16, CC.B_WIDTH, CC.B_HEIGHT *9/16);
			else if (this.get_energy() <= 0.5 * CC.ENERGY_MAX)
				this.sim.image(Simulator.battery2, this.address.x * Simulator.X_DISP, this.address.y * Simulator.Y_DISP + CC.B_HEIGHT *9/16, CC.B_WIDTH, CC.B_HEIGHT *9/16);
			else if (this.get_energy() <= 0.75 * CC.ENERGY_MAX)
				this.sim.image(Simulator.battery3, this.address.x * Simulator.X_DISP, this.address.y * Simulator.Y_DISP + CC.B_HEIGHT *9/16, CC.B_WIDTH, CC.B_HEIGHT *9/16);
			else // this.get_energy() <= 1.0 * Instruction.ENERGY_MAX)
				this.sim.image(Simulator.battery4, this.address.x * Simulator.X_DISP, this.address.y * Simulator.Y_DISP + CC.B_HEIGHT *9/16, CC.B_WIDTH, CC.B_HEIGHT *9/16);
		}
	}
	...
	public void generate_sound() {
		this.sim.detector(this.address.x, this.address.y, CC.SOUND_RANGE, CC.SOUND);
	}
    ...
    public void parent_reproduce() {
    	this.sim.reproduce_biolume(this.address, this.genome, this.exe_size, CC.REPLACEMENT_METHOD, CC.REPLICATION_METHOD);
    	this.repro_cnt = CC.REPRO_COUNTDOWN;
    	this.is_child = false;
	}
	...
	public void child_reproduce(int[] new_genome, int new_exe_size, boolean step_after) {
		...
		if (step_after) {
			...
			this.repro_cnt = CC.REPRO_COUNTDOWN + 1;
		}
		else this.repro_cnt = CC.REPRO_COUNTDOWN;
	...
	private void mutate() {
		...
		for (int i = CC.EXE_SIZE_MAX; i < GENOME_SIZE_MAX; i++) {
			if (RAND.nextDouble() < CC.MUT_PROB_COPY) {
			    ...
			    if (i == CC.EXE_SIZE_MAX || i == (CC.EXE_SIZE_MAX+3) ) { // hues
		...
		if (RAND.nextDouble() < CC.MUT_PROB_INSERT && this.exe_size < CC.EXE_SIZE_MAX) {
			this.exe_size++;
	...
	private void makeViable() {
		...
		if (this.exe_size < CC.EXE_SIZE_MAX) { // perform an insertion mutation
			this.exe_size++;
	...
	public boolean getSensorDetected(int sensor_type, int term) {
		if(term == CC.SHORT) return (this.sensor_countdowns[sensor_type] == CC.LONG_TERM_DURATION);
		else if (term == CC.LONG) return (this.sensor_countdowns[sensor_type] != 0);
		else {
	...
	public void setSensorDetected(int sensor_type) {
		if (sensor_type == CC.TOUCH && !this.getSensorDetected(CC.TOUCH, CC.SHORT)) {
			this.add_energy(CC.TOUCH_ENERGY);
			this.sensor_countdowns[CC.TOUCH] = CC.LONG_TERM_DURATION;
		}
		else if (sensor_type == CC.MOTION && !this.getSensorDetected(CC.MOTION, CC.SHORT)) {
			this.add_energy(CC.MOTION_ENERGY);
			...
			if (this.sensor_countdowns[CC.SOUND] < CC.LONG_TERM_DURATION - CC.NEW_SOUND_DELAY) 
					this.add_energy(CC.NEW_SOUND_ENERGY);
			else this.add_energy(CC.NORMAL_SOUND_ENERGY); // multiple sounds make more "loudness"
			this.sensor_countdowns[CC.SOUND] = CC.LONG_TERM_DURATION;
		}
		else if (sensor_type == CC.CO2 && !this.getSensorDetected(CC.CO2, CC.SHORT)) {
			this.add_energy(CC.CO2_ENERGY);
