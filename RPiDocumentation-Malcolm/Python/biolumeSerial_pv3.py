...
import constants_pv3 as constants
...
class Biolume:
	...
	Properties:
		...
		camera
			An PyGame object used to capture frames from the camera
		imageSize
		...
		windowName
			A window for displaying the resulting image
	...
	def initialize_motion_detection(self):
		...
		self.imageSize = (80, 60) # this size is unsupported by the ps eye so images must be converted after they are grabbed
		self.numPixels = 80 * 60
	
		
		#create fullscreen display
		self.window = pygame.display.set_mode(self.imageSize, 0)

		#find, open and start camera
		cam_list = pygame.camera.list_cameras()
		self.camera = pygame.camera.Camera(cam_list[0], self.imageSize)
		self.camera.start()
	
	
	def destroy_motion_detection(self):
		"""Shutdown the objects used for motion detection."""
	...
	def rgbToGray(self, rgb):
		...
		Parameters:
			...
			rgb
				A numpy array containing an RGB image
	...
	def detect_motion(self):
		...
		#grab image and scale to the desired resolution
		self.colorFrame = pygame.transform.scale(self.camera.get_image(), self.imageSize)
		
		# convert pygame.Surface to numpy array
		self.colorFrame = pygame.surfarray.array3d(self.colorFrame)
		
		# process image and detect motion
		self.process_image()
		self.motionDetected = self.is_motion_detected1()
		
		#draw all updates to display
		if self.resultImage is not None:
			pygame.surfarray.blit_array(self.window, self.resultImage)
	...
	def process_image(self):
		...
		self.currentGrayFrame = self.rgbToGray(self.colorFrame)
		
		# remove noise, etc.
		self.currentGrayFrame = self.reduce_image_noise(self.currentGrayFrame)
		
		if self.previousGrayFrame is not None:
			# find the difference between the current and previous frame
			self.resultImage = np.subtract(self.currentGrayFrame, self.previousGrayFrame)
			
			# calculate the binary image that shows where there is change in the image
			self.resultImage = np.where(((self.resultImage>10)|(self.resultImage<-10)), 255, 0)
		
		self.previousGrayFrame = self.currentGrayFrame
	
	
	def is_motion_detected1(self):
		"""Check whether or not motion is detected in the most recent image and return a boolean."""
		
		numChanged = np.count_nonzero(self.resultImage) # count the number of pixels that have changed since the last frame
		
		percChanged = (numChanged*100) / self.numPixels # calculate the percent of pixels that are changed
		
		# if the percent is over the threshold, movement is detected
		if (percChanged) > constants.MOTION_THRESHOLD:
			return True
		else:
			return False
